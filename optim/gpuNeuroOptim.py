...
class gpuOptim:
    ...
    def __init__(self):
        ...
        self.best_cost = 1.0; #globally best cost
    ...
    def optimise(self):
        ...
        f1,f2 = self.getObjcosts();
        self.sortSolutions(f1);
        self.sortSolutions(f2);
        print "f1:",f1;
        print "f2:", f2;
    ...
    def gpu_diffEvolve(self):
        ...
        one_random_sol = 1;
        sel_sols = self.pick(min_sols_req,True);
    ...
    def crossOver(self, pop = None):
        ...
        cand_sols = self.pick(num_req_sols,True);
        ...
        if cand_sols != None:
            ...
            if self.debug == True:
                ...
                print "parentA:", solAi;
                print "len A:", len(solAi);
                print "parentB:", solBi;
                print "len B:", len(solBi);
            ...
            #get solutions
            solA = solAi[crossOverFrom:];
            solB = solBi[crossOverFrom:];
            ...
            if(prCO <= self.prob_cross_over ):
                ...
                trialSol = solAi[constants.META_INFO_COUNT : crossOverFrom];
    ...
    def gauss_mutate(self,pop=None):
        ...
        sols = self.pick(req_num_sol,True);
        #get solution
        sol = sols.pop();
        #copy
        nwSol = list(sol);
    ...
    def pick(self,N,pick_random = False ,pop = None ):
        """ picks N solutions from the population with the given same lengths
            -opimised method of picking similar solutions from the population based on hash tables
        """
        ...
        if pick_random:
            ...
            #sort by test error (error after noise treatment)
            self.sortSolutionsByTestError();
        ...
        for sol in getGenerator(pop):
            #get the length
            k = len(sol);
            
            if len_hashtable.has_key(str(k)):
                #add to list of solutions with 
                len_hashtable[str(k)].append(sol);
            else:
                if self.debug:
                    print "no key";
                #create a new key and add to list
                len_hashtable[str(k)] = [];
                len_hashtable[str(k)].append(sol);
    
            #stop if there is a sufficient number of solutions (as requested)
            if len(len_hashtable[str(k)]) == N:
                if self.debug:
                    print "Found"
                #return the solutions and their indices
                picked_indx =[];
                picked_indx.extend(len_hashtable[str(k)]);
                break;
