...
ControlFiles = ['card.csv', 'heart.csv', 'diabetes.csv', 'cancer.csv', 'iris.csv'];
Control_results = {
...
for i,file_name in enumerate(NoinjectFiles):
    ...
    NoinjectFiles['test_errs'][i]= test_err;
    NoinjectFiles['train_errs'][i]=train_err;
    NoinjectFiles['train_errs_std'][i] = train_err_std;
    NoinjectFiles['test_errs_std'][i] = test_err_std;
...
##                    label = 'train');
    ...
    plt.title('$Performance (Error) - No\ Injections $');
    plt.xticks(ind+width, Datasets);
##    plt.legend();

    #plot and save
    plt.tight_layout();
    plt.savefig('errs_no_inject'+'.png');
    plt.show();
