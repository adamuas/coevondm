...
print ">> Setting Parameters -- TF";
#Optimisation params - BIAS
params = dict();
params['targer_err'] = 0.00;
params['NGEN'] = 5;
### NET CONFIG
params['numI'] = 4; #number of input nodes
...
print ">> Setting Parameters -- optim";
...
def gen_random_nodes(layer, n = 5):
    """
...
def gen_temporal_nodes(layer, n = 5):
    """
...
def evaluate(components, verbose = False, copy = True, eval_type = None,**kwargs):
    """
...
def evaluate_map(components, verbose = False, copy = True, eval_type = None,**kwargs):
    """
    Evaluate a given solution of components from all species
    """
...
def evaluate_connIH_map(iter,connIH):
    """
    evaluates an input-hidden connection topology in parallel using dtm - (MPI)
    """

    #get the representative
    represntatives = shared.getConst('represntatives_'+str(iter));
...
def evaluate_connHH_map(iter,connHH):
    """
...
def evaluate_connHO_map(iter,connHO):
    """
...
def evaluate_weightIH_map(iter,weightIH):
    """
...
def evaluate_weightIH(next_gen):
    """
...
def evaluate_weightHH_map( iter, weightHH):
    """
    Evaluate the hidden- hidden units in parallel
    """
    #get the representative
    represntatives = shared.getConst('represntatives_'+str(iter));
...
def evaluate_weightHH(next_gen):
    """
...
def evaluate_weightHO_map( iter, weightsHO):
    """
...
def evaluate_weightHO( next_gen):
    """
    evaluates weights from hidden to output
    """
...
def evaluate_model_map(iter, model):
    """
...
def evaluate_hiddenNodes_map(iter,h_node, hi):
    """
    implementation that works using mpi for parallel computing
    """

    #get the recent representative
    represntatives = shared.getConst('represntatives_'+str(iter));
...
def evaluate_hiddenNodes(next_gen,hi):
    """
...
def evaluate_outputNodes_map(iter,o_node, oi):
    """
    """
...
def evaluate_outputNodes(next_gen, oi):
    """
...
if __name__ == '__main__':
    ...
    #Create the queue for the best cost
    best_cost_queue = Queue();
    ...
    ### Optimisation ###
    print ">>Selecting Representative";
    ...
    for current_iter in xrange(params['NGEN']):
        ...
        print "SHARING REPRESENTATIVES FOR GEN {0}".format(current_iter);
        setConstParams = {'representatives_'+str(current_iter): represntatives};
        shared.setConst(**setConstParams);
        ...
        for s_key, specie in pop.iteritems():
            ...
            if s_key == 'hidden_nodes':
                ...
                for hi,h_nodes in specie.iteritems():
                    ...
                    h_nodes = futures.map(evaluate_hiddenNodes_map, [current_iter for x in xrange(len(h_nodes))], h_nodes,hi);
            ...
            if s_key == 'out_nodes':
                ...
                for oi,o_nodes in specie.iteritems():
                    ...
                    o_nodes = futures.map(evaluate_outputNodes_map, [current_iter for x in xrange(len(o_nodes))],o_nodes, oi );
            ...
            if s_key == 'model_pop':
                #evaluate the models
                specie = futures.map(evaluate_model_map, [current_iter for x in xrange(len(specie))], specie);
            ...
            if s_key == 'connActive_IH_pop':
                specie = futures.map(evaluate_connIH_map, [current_iter for x in xrange(len(specie))], specie);
            ...
            if s_key == 'connActive_HH_pop' \
                ...
                specie = futures.map(evaluate_connHH_map, [current_iter for x in xrange(len(specie))], specie);
            ...
            if s_key == 'connActive_HO_pop':
                #evaluate and score
                specie = futures.map(evaluate_connHO_map, [current_iter for x in xrange(len(specie))], specie);
            ...
            if s_key == 'connWeights_IH_pop':
                ...
                #evaluate and map
                specie = futures.map(evaluate_weightIH_map, [current_iter for x in xrange(len(specie))], specie);
            ...
            if s_key == 'connWeights_HH_pop' \
                ...
                #evaluate and map
                specie = futures.map(evaluate_weightHH_map, [current_iter for x in xrange(len(specie))], specie);
            ...
            if s_key == 'connWeights_HO_pop':
                ...
                specie = futures.map(evaluate_weightHO_map, [current_iter for x in xrange(len(specie))], specie);
